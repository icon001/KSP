...
begin
  ...
  end else if (aCardType = CNU811) then
  begin
    aCardNo := FillCharStr(aCardNo,'*',15);
  end;
...
begin
  ...
  else if (Value = 'ICU-310') then DeviceType := ICU310
...
//LogSave(FormatdateTime('yyyymmdd',now)+'.log','DeviceType:' + ECUID + Value);
  FDeviceType := Value;
  ArmExtension := False;
  MaxECUCount := 63;
  ...
  begin
    ...
    ScheduleUse := False;
    MaxCardCount := 1000;
  end else if (Value = ICU300) or(Value = ICU310) then
  begin
    MaxDoorNo := 1;
    MaxReaderNo := 0;
    MaxPortNo := 6;
...
begin
  ...
  else if Pos('CNU',stDeviceVersion) > 0 then CardType := CNU811 //충남대용
...
var
  ...
  bUpdate : Boolean;
begin
//  LogSave(FormatdateTime('yyyymmdd',now)+'.log','SetReaderInfo:' + ECUID + aReaderNo + aUsedCR);
  bUpdate := False;
...
procedure TCurrentDeviceState.SetReaderUse(aReaderNo: integer;
  aReaderUse: Boolean);
begin
//  LogSave(FormatdateTime('yyyymmdd',now)+'.log','SetReaderUse(1):' + ECUID + inttostr(aReaderNo) + inttostr(MAXREADERNO) );
  if aReaderNo > MAXREADERNO then Exit;
  if aReaderUse then ReaderUse[aReaderNo] := '1'
  else ReaderUse[aReaderNo] := '0';
//  LogSave(FormatdateTime('yyyymmdd',now)+'.log','SetReaderUse:' + ECUID + inttostr(aReaderNo) + ReaderUse[aReaderNo]);
end;

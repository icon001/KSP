...
type
  ...
  public
    ...
    procedure CurrentDeviceReaderConnectChange(aEcuID,aNo:string;aConnected:Boolean);
  end;
...
procedure TfmNetworkCheck.CurrentDeviceReaderConnectChange(aEcuID,
...
begin
  ...
  ReaderNo := strtoint(aNo);
  for i:=2 to sg_NetworkState.RowCount - 1 do
  begin
    if sg_NetworkState.Cells[0,i] = aEcuID then
    begin
      if aConnected then
      begin
        sg_NetworkState.Cells[readerno + 1,i] := '성공';
        sg_NetworkState.Colors[readerno + 1,i] := clGreen;
      end else
      begin
        sg_NetworkState.Cells[readerno + 1,i] := '실패';
        sg_NetworkState.Colors[readerno + 1,i] := clRed;
      end;
      break;
    end;
  end;

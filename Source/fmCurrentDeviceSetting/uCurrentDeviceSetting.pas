      ...
      begin
        ...
        begin
          ...
          begin
            ...
            aList.ImageIndex:= -1;
            nDeviceType := ord(FindField('DEVICE_TYPE').AsString[1]) - ord('0');
            if nDeviceType <= DeviceTypeNameList.Count then
              aList.SubItems.Add(DeviceTypeNameList.Strings[nDeviceType]);
          end;
          if aIndex > -1 then
          begin
...
begin
  ...
  if TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = ACC100 then CardReaderTypeLoad(1)
  ...
  Try
    ...
    begin
      ...
      begin
        ...
        if (G_stDeviceType[strtoint(G_stSelectEcuID)] = KTT811) OR (G_stDeviceType[strtoint(G_stSelectEcuID)] = KTT813)  then
        begin
...
begin
  ...
  if (G_stDeviceType[strtoint(G_stSelectEcuID)] = ICU100) or
     (G_stDeviceType[strtoint(G_stSelectEcuID)] = ICU200) then Exit; //KTT811System 이 아니면 빠져 나가자...
...
begin
  ...
  if (TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = KTT811) or
     (TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = KTT812) then
...
begin
  ...
  if isDigit(stDeviceType) then
    cbDeviceType.ItemIndex := strtoint(stDeviceType) - 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        stDeviceType := '';
        nDeviceType := ord(G_stDeviceType[strtoint(DeviceList.Strings[i])][1]) - ord('0');
        if nDeviceType <= DeviceTypeNameList.Count then
          stDeviceType := DeviceTypeNameList.Strings[nDeviceType];
        if stDeviceType = '' then stDeviceType := 'Type Checking...';
...
begin
  cmb_EcuType.Clear;
  for i := 1 to DeviceTypeNameList.Count - 1 do
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Not L_bTableLoading then
      begin
        dmDataBase.UpdateTB_DEVICE_FieldName(G_stSelectEcuID,'DEVICE_TYPE',inttostr(ItemIndex + 1));
        dmDataBase.UpdateTB_DEVICERCV_FieldName(G_stSelectEcuID,'DEVICE_TYPE','U');
      end;
      if isDigit(stEcuID) then G_stDeviceType[strtoint(stEcuID)] := inttostr(ItemIndex + 1) ;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    else if (aDeviceCode = 'ICU-310') then stDeviceName := 'ICU-310'
...
begin
  ...
  case aCardType[1] of
      ...
      CNU811      : st_CardType.Caption := '충남대용';
  end;
...
begin
  ...
  cb_CardType.Items.Add('일반리더(CRO-811)');
  for i := 1 to 8 do
  begin
    cmbBox := TravelComboBoxItem(gb_CardReader,'cmb_ReaderType' , i);
    if cmbBox <> nil then cmbBox.Items.Add('일반리더(CRO-811)');
  end;
  cb_CardType.Items.Add('지문리더(CRO-801)');
  for i := 1 to 8 do
  begin
    cmbBox := TravelComboBoxItem(gb_CardReader,'cmb_ReaderType' , i);
    if cmbBox <> nil then cmbBox.Items.Add('지문리더(CRO-801)');
  end;

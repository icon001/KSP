...
begin
  ...
  if (G_bIntercept) and (G_nConnectMode = 0) and (G_nServerMode = 0) then //LAN 접속이면서 Client 모드 이면
  begin
    if Not bResult then
    begin
...
//  LogSave(FormatdateTime('yyyymmdd',now)+'.log','DeviceListAdd:' + stEcuID);
...
//  LogSave(FormatdateTime('yyyymmdd',now)+'.log','DeviceListDelete:' + stEcuID);
    end;
  end;
      ...
      begin
        ...
        begin
          case aDeviceType[i] of
            ...
            '8' : TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType := SGS100;
            '9' : TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType := ICU310;
          end;
...
begin
  ...
  DeviceTypeNameList.Add('ECU-400');
  DeviceTypeNameList.Add('ICU-310');
end;
...
begin
  ...
  begin
    ...
    TfmNetworkCheck(fmNetworkCheck).CurrentDeviceReaderConnectChange(aEcuID,aNo,aConnected);
  end;  
...
begin
  ...
  begin
    ...
    end else if (aDeviceType = KTT811) or (aDeviceType = KTT813) or (aDeviceType = SGS100) then
    begin
      mn_FirmWareUpdate.Visible := True;
    end;
  end;
...
begin
  ...
  begin
    ...
    end else if stOldCardType = CNU811 then
    begin
      if TCurrentDeviceState(DeviceList.Objects[i]).CardType <> NOTHINGTYPE then
      begin
        if TCurrentDeviceState(DeviceList.Objects[i]).CardType <> CNU811 then Exit;
      end;
...
begin
  ...
  Try
    ...
    if (TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = KTT811) or (TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = KTT813)
       or (TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = SGS100) then
    begin
      //시리얼 접속인지 체크
      if G_nConnectMode = 1 then
      begin
...
begin
  ...
  if nIndex < 0 then Exit;
  Delete(aData,1,1);
//  LogSave(FormatdateTime('yyyymmdd',now)+'.log','CardReaderStateData:' + aECUID + aData);

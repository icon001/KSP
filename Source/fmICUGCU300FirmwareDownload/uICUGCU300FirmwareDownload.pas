...
begin
  ...
  begin
    ...
    begin
      ...
      end else if DeviceType = 'ICU310' then
      begin
        if TCurrentDeviceState(DeviceList.Objects[nIndex]).DeviceType = ICU310 then
        begin
          if TCurrentDeviceState(DeviceList.Objects[nIndex]).Connected then
          begin
            Group_811.ItemChecked[i]:= True;
...
begin
  ...
  if (Value = 'ICU300') or (Value = 'ICU310') then
  begin
...
var
  ...
  i : integer;
begin
  if (DeviceType = 'ICU300') or (DeviceType='ICU310') then
  begin
...
begin
  ...
  end else if (stDeviceType = 'ICU300') or (stDeviceType = 'ICU310')  then
  begin
...
procedure TfmICUGCU300FirmwareDownload.MainRequestProcessChange(
  ...
  aCurPosition: integer);
begin
  if (aDeviceType = 'ICU300') or (aDeviceType = 'ICU310') then
